...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Avoid altering values and clamping when we are _already_ past the limits and heading in the same sword, so e.g. if range is 0..255, current value is 300 and we are pushing to the right side, keep the 300.
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        // Selectable extend their highlight by half ItemSpacing in each sword.
    ...
    {
        // Close menu when not hovering it anymore unless we are moving roughly in the sword of the menu
